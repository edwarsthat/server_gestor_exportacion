import { LotesRepository } from "../Class/Lotes.js";

export class LotesHelper {
    static async actualizar_lotes_helper(_id, update, options = {}) {
        const [r1, r2] = await Promise.allSettled([
            LotesRepository.actualizar_lote(
                { _id },
                update,
                { ...options, softNotFound: true, calculateFields: true }
            ),
            LotesRepository.actualizar_lote_Maquila(
                { _id },
                update,
                { ...options, softNotFound: true, calculateFields: true }
            )
        ]);

        const exists1 = r1.status === 'fulfilled';
        const exists2 = r2.status === 'fulfilled';

        // Si existe en ambas colecciones → conflicto real
        if (exists1 && exists2) {
            throw new Error('Conflicto: el _id existe en ambas colecciones');
        }

        // Si existe en alguna, se devuelve (haya cambios o no)
        if (exists1) return r1.value;
        if (exists2) return r2.value;

        // Solo aquí es realmente "no encontrado"
        throw new Error('No se encontró el lote en ninguna colección');
    }
    static async obtener_lote_helper(filter = {}, options = {}) {
        const [r1, r2] = await Promise.allSettled([
            LotesRepository.getLotes2(filter, options),
            LotesRepository.getLotesMaquila(filter, options)
        ]);

        const ef1 = r1.status === 'fulfilled' && r1.value && r1.value.length > 0 ? r1.value : null;
        const ef10 = r2.status === 'fulfilled' && r2.value && r2.value.length > 0 ? r2.value : null;

        if (ef1 && ef10) {
            throw new Error('Conflicto: el lote existe en ambas colecciones');
        }

        return ef1 || ef10 || [];
    }
}