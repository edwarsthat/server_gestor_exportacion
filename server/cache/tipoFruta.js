...
export class tipoFrutaCache {
    static async cargar(reintentos = 5, delayMs = 1000) {
        for (let intento = 1; intento <= reintentos; intento++) {
            try {
                ...
                for (let i = 0; i < tipoFruta.length; i++) {
    ...
    static getTiposFruta() {
        try {
            return tipoFrutaMap;
        } catch (err) {
            console.error(`[CACHE] Error al obtener tipos de fruta:`, err.message);
            throw new ConnectRedisError(502, `Error al obtener tipos de fruta: ${err.message}`);
        }
    }
